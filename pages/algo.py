...
def mutate(individual, mutation_rate, green_min, green_max):
    ...
    for _ in range(int(mutation_rate * num_lights)):
        ...
        # Adaptive mutation step size proportional to mutation rate
        sigma = np.random.choice([-1, 1]) * mutation_rate * (green_max - green_min) * np.random.rand()
...
def genetic_algorithm(pop_size, num_lights, max_iter, green_min, green_max, cycle_time, mutation_rate, pinv, beta, cars, verbose=False):
    ...
    for iteration in range(max_iter):
        ...
        # Adaptive mutation rate: decrease over iterations
        adaptive_mutation_rate = mutation_rate * (1 - iteration / max_iter)

        # Elitism: carry over top 5% unchanged
        ...
        while len(new_population) < pop_size:
            ...
            if np.sum(child1) <= cycle_time:
                child1 = mutate(child1, adaptive_mutation_rate, green_min, green_max)
            ...
            if np.sum(child2) <= cycle_time:
                child2 = mutate(child2, adaptive_mutation_rate, green_min, green_max)
        ...
        # Apply inversion to fill population if needed
        while len(new_population) < pop_size:
            i = np.random.randint(0, len(population))
            individual = inversion(population[i][0], num_lights)
            if np.sum(individual) <= cycle_time:
                individual = mutate(individual, adaptive_mutation_rate, green_min, green_max)
        ...
        # Sort population by fitness (delay)
        ...
        if verbose:
            print(f"Iteration {iteration+1}: Best Total Delay = {best_sol[1]}")
            print(f"Green Times: North = {best_sol[0][0]}, South = {best_sol[0][1]}, West = {best_sol[0][2]}, East = {best_sol[0][3]}")
    
    return best_sol, best_delays

def ml_optimize_traffic(cars):
    """
    Placeholder for ML-based traffic optimization.
    cars: list or array of current car counts per light [N, S, W, E]
    Returns dict of optimized green times.
    """
    # For demonstration, return equal green times
    avg_time = 30
    return {
        'north': avg_time,
        'south': avg_time,
...
def optimize_traffic(cars, method='genetic', pop_size=400, num_lights=4, max_iter=25, green_min=10, green_max=60, cycle_time=148, mutation_rate=0.02, pinv=0.2, beta=8, verbose=False):
    """
    Optimize traffic light green times using specified method.
    method: 'genetic' or 'ml'
    cars: list or array of current car counts per light [N, S, W, E]
    Returns dict of optimized green times.
    """
    if method == 'ml':
        return ml_optimize_traffic(cars)
    else:
        best_sol, best_delays = genetic_algorithm(pop_size, num_lights, max_iter, green_min, green_max, cycle_time, mutation_rate, pinv, beta, cars, verbose=verbose)
...
def detect_cars(video_path):
    # Placeholder implementation for car detection
    # Replace this with actual car detection logic using a model or OpenCV
    print(f"Detecting cars in video: {video_path}")
    # For now, return a dummy number of cars
    return 10
